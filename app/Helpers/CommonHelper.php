...
if (!function_exists('loginResponse')) {
    function loginResponse($user)
    {
        ...
        {
            return new IndividualLoggedInResource($user);
        }
        ...
        if ($user->role == 'company')
        {
            ...
            return new CompanyLoggedInResource($user);
        }
    }
}
...
if (!function_exists('makeThumbnail')) {
    ...
    function makeThumbnail($video_name)
    {
        $file_thumb_ = "";

        $extension_video = explode("/",$video_name->getClientMimeType());
        if($extension_video[0] == "video"){
            $file_thumb_image = mt_rand().time().".png";
            $thumbnail = getcwd()."/thumb/".$file_thumb_image;
            $cmd = sprintf('ffmpeg -i %s -ss %s -f image2 -vframes 1 %s', $video_name, 05, $thumbnail);
            exec($cmd);
            $file_thumb_ = "/thumb/".$file_thumb_image;
        }
        return $file_thumb_;
    }
}
...
if (!function_exists('getVideoUrl')) {
    ...
    {
        $path = config('constants.VIDEO_URL');
        return url($path) . '/' . $filename;
    }
}
