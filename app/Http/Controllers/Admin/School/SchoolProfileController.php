...
class SchoolProfileController extends Controller
{
    public function DashboardUpdateProfile(Request $request)
    {
        ...
        {
            auth('school')->user()->name = $request->user_name;
        }

        if ( $request->exists('phone') )
        {
            auth('school')->user()->phone = $request->phone;
        }
    ...
    public function changePassword(Request $request)
    {
        ...
        auth('school')->user()->password = bcrypt($new_password);
        auth('school')->user()->save();
    ...
    public function ChangeImage(Request $request)
    {
      ...
      if ($request->hasFile('image')) {
          ...
          $imageName = $uuid . time() . '.' . $request->file('image')->getClientOriginalExtension();
          $request->file('image')->move(public_path('/uploadedimages'), $imageName);
          auth('school')->user()->image = asset('public/uploadedimages') . "/" . $imageName;
          auth('school')->user()->save();
