                  ...
                  <div class="gentableWrap mt-2 col-8">
                     ...
                     @if($chat_list &&  count($chat_list) > 0 )
                        ...
                        <div class="notificationItem"> 
                           ...
                           <a href="{{ route('school_chat',[$chat->user_id,  $chat->role] ) }}"  class="textBox">
                              <span>
        ...
        url: "{{ route('admin_search_users') }}",
         ...
         success: function(result) {
            ...
            if (result)
            {
               html = '';
               $("#users_list").remove();
               html = `<div id="users_list">`;
               if ( result.length > 0 )
               {
                  $.each(result, (key, value) => {
                     var url = '{{ route("school_chat", [":id" , ":role"]) }}';
                     url = url.replace(':id', value.id);
                     url = url.replace(':role', value.role);
                     html += `<a href="${url}"  >${value.name} </a> `;
                  });
